...
template <typename T>
SLL<T>::SLL()
{ // Default constructor for SLL, set head to null.
    head = nullptr;
}

template <typename T>
void SLL<T>::addFront(T elem)
{ // Add to the front of the SLL.
    if (head == nullptr)
    { // If empty, set as new head.
        Node<T> *newNode = new Node<T>(elem);
        head = newNode;
        newNode->next = nullptr;
    }
    else
    { // If not, set newNode to the front and its next node to the previous head.
        Node<T> *temp = head;
        Node<T> *newNode = new Node<T>(elem);
        head = newNode;
        newNode->next = temp;
...
{               // Insert a node at a given index.
    ...
    { // Use pointers to increment to correct index, with prev being the previous node.
        Node<T> *temp = head;
        Node<T> *prev = head;
        Node<T> *newNode = new Node<T>(elem);
...
{               // Remove a node at an index (similar to insertAt).
...
template <typename T>
Node<T> *SLL<T>::front()
{ // Return the front of the SLL.
    return head;
}

template <typename T>
Node<T> *SLL<T>::at(int n)
